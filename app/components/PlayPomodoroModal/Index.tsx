...
export const PlayPomodoroModal: React.FC<Props> = (props: Props) => {
  ...
  const [playPomodoroState, setPlayPomodoroState] = useState<
    "play" | "pause" | "stop"
  >("stop")
  ...
  const [pomodoroElementsState, setPomodoroElementsState] = useState<
    {
      mode: string
      playlist_id: string
      time: number
  ...
  const [backgroundColor, setBackgroundColor] = useState<string>(
    "from-blue-800/90 to-green-800/90"
  )
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < pomodoro.term_repeat_count; i++) {
      for (let j = 0; j < pomodoro.term_count; j++) {
        pomodoroElements.push({
          mode: "work",
          playlist_id: pomodoro.work_time_playlist_id,
          time: pomodoro.work_time,
          term: j + 1,
          term_repeat: i + 1,
        })
        if (j !== pomodoro.term_count - 1) {
          pomodoroElements.push({
            mode: "break",
            playlist_id: pomodoro.break_time_playlist_id,
            time: pomodoro.break_time,
            term: j + 1,
            term_repeat: i + 1,
          })
        } else {
  ...
  useEffect(() => {
    ...
    pomodoroElementsState.forEach((pomodoroElement, index) => {
      changeTrackTime.push(
        changeTrackTime[changeTrackTime.length - 1] + pomodoroElement.time
      )
    })
  ...
  useEffect(() => {
    ...
    currentPomodoroPosition,
    isRunningCountupTimer,
    pomodoroElementsState,
    goNextTrack,
  ])
  ...
  useEffect(() => {
    ...
    setCurrentTermRepeat(
      pomodoroElementsState[currentPomodoroPosition].term_repeat
    )
  ...
  useEffect(() => {
    ...
    if (
      currentTermRestTime === 3000 ||
      currentTermRestTime === 2000 ||
      currentTermRestTime === 1000
    ) {
      beep()
    }
  }, [currentTermRestTime])
    ...
    <>
      {open && (
        <div
          ...
          } transform fixed top-0 left-0 w-full h-full bg-gradient-to-br items-center justify-center`}
        >

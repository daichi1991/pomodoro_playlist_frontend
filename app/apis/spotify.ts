...
export const transferPlayback = async (device_id: string, retry: boolean = false): Promise<void> => {
  console.log('transferPlayback', device_id);
...
export const startPlaylist = async (playlist_id: string, retry: boolean = false): Promise<void> => {
  console.log('startPlaylist', playlist_id)
...
export const getPlaylists = async (retry: boolean = false): Promise<SpotifyPlaylistItems[] | []> => {
  const access_token = localStorage.getItem('access_token');
  if (!access_token) return [];
  const requestHeader = {
    'Authorization': 'Bearer ' + access_token,
...
export const getSpotifyUserId = async (retry: boolean = false): Promise<string | null> => {
  const access_token = localStorage.getItem('access_token');
